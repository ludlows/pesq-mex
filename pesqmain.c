...
void mexFunction(int nlhs, mxArray *plhs[], int nrhs, const mxArray *prhs[]){
    ...
    argv[i] = mxMalloc( 1 );
    argv[i] = "v";
    reference_signal = (float *)mxGetData(prhs[i-1]);
    refLen = mxGetNumberOfElements(prhs[i-1]);    
    i = (argc-1);
    argv[i] = mxMalloc( 1 );
    argv[i] = "v";
    degraded_signal = (float *)mxGetData(prhs[i-1]);
    degLen = mxGetNumberOfElements(prhs[i-1]);
    
    // the dummy path "v", 
    // is useless placeholder to take place of orginal path 
   
    res = compute(argc, argv, 
            reference_signal, refLen, 
            degraded_signal, degLen);
